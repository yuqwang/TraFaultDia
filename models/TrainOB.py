import argparse
import torch
from DatasetOB import DatasetOB
import numpy as np
import random
...
def train_maml(config):
    ...
    db = DatasetOB(batchsz=config['task_num'], n_way=config['n_way'], k_spt=config['k_spt'], k_query=config['k_qry'],
                   train_span_max_length=config['TTspan_max_length'], train_log_max_length=config['TTlog_max_length'],
                   train_span_input_dim = config["TTspan_input_dim"], train_log_input_dim = config["TTlog_input_dim"],
                   test_span_max_length=config['BTspan_max_length'], test_log_max_length=config['BTlog_max_length'],
                   test_span_input_dim=config["BTspan_input_dim"], test_log_input_dim=config["BTlog_input_dim"],
                   )
    ...
    for epoch in range(config['epoch']):
        ...
        x_spt_tensor, x_qry_tensor, y_spt_tensor, y_qry_tensor = db.load_data_cache('train')
...
if __name__ == '__main__':
    ...
    search_space = {
        ...
        "k_qry": 15,
        "task_num": 4,
        #"update_lr": tune.loguniform(2e-3, 9e-2),
        "update_lr": tune.loguniform(2e-3, 5e-2),
        #"meta_lr": tune.loguniform(1e-5, 1e-3),
